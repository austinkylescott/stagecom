...
import {
  ...
  useTheaterSchedule,
} from "~/composables/useTheaterSchedule";

type TimelineFilter = TheaterScheduleParams["timeline"];
...
const timelineFilter = ref<TimelineFilter>(
  routeTimeline === "upcoming" || routeTimeline === "past" ? routeTimeline : "upcoming",
);
...
const scheduleParams = computed<TheaterScheduleParams>(() => ({
  slug: slug.value,
...
const { data: initialDetails } = await useAsyncData(
  `theater-calendar-details-${slug.value}`,
  () =>
...
const { data: theaterData } = useTheaterDetails(slug, initialDetails);

const { data: initialSchedule } = await useAsyncData(
  `theater-calendar-${slug.value}-${scheduleParams.value.month}-${scheduleParams.value.timeline}`,
  () =>
    $fetch<TheaterScheduleResponse>(`/api/theaters/${slug.value}/schedule`, {
      headers: import.meta.server ? useRequestHeaders(["cookie"]) : undefined,
      credentials: "include",
      params: {
...
const { data, isLoading, error } = useTheaterSchedule(scheduleParams, initialSchedule);

const canReview = computed(() => theaterData.value?.permissions?.canReview ?? false);
const isMember = computed(() => theaterData.value?.membership?.status === "active");
const theaterName = computed(
  () => theaterData.value?.theater.name || data.value?.theater.name || slug.value,
);
...
const agendaItems = computed(() =>
  allItems.value.slice().sort(
    (left, right) =>
      new Date(left.startsAt).getTime() - new Date(right.startsAt).getTime(),
  ),
);
const nextUpItem = computed(() => agendaItems.value[0] ?? null);
const todayCount = computed(() => itemsByDate.value.get(todayDate)?.length ?? 0);
...
const routeQuery = computed(() => ({
  ...
  timeline: timelineFilter.value !== "upcoming" ? timelineFilter.value : undefined,
}));
...
watch(
  ...
  (query) => {
    ...
    const nextTimeline =
      query.timeline === "all" || query.timeline === "past"
        ? query.timeline
        : "upcoming";
  ...
  <div class="space-y-0">
    ...
    >
      <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem] lg:items-start">
        <div class="space-y-5">
          <span class="stage-kicker">Theater calendar</span>
          <div>
            <h1 class="stage-section-title">{{ theaterName }}</h1>
            <p class="mt-3 max-w-3xl text-lg leading-8 stage-muted">
              Use this as the theater-scoped programming board: month scan, selected-day detail, and the full agenda without dropping into the broader cross-theater schedule.
            </p>
          </div>

          <div class="grid gap-3 sm:grid-cols-3">
            <div class="stage-stat">
              <span class="stage-overline">Loaded items</span>
              <span class="stage-stat-value">{{ agendaItems.length }}</span>
              <p class="mt-2 text-sm stage-muted">Occurrences matching the current calendar filters.</p>
            </div>
            <div class="stage-stat">
              <span class="stage-overline">Today</span>
              <span class="stage-stat-value">{{ todayCount }}</span>
              <p class="mt-2 text-sm stage-muted">Items landing on today's date.</p>
            </div>
            <div class="stage-stat">
              <span class="stage-overline">Review queue</span>
              <span class="stage-stat-value">{{ theaterData?.stats.pendingReviewCount ?? 0 }}</span>
              <p class="mt-2 text-sm stage-muted">Pending theater decisions, when visible to you.</p>
            </div>
          </div>
        </div>
        ...
        <UCard class="stage-dot-board">
          ...
          <div v-if="nextUpItem" class="space-y-3 text-sm text-[var(--stage-ink)]">
            ...
            <div class="flex flex-wrap gap-2">
              <UButton color="warning" :to="`/theaters/${slug}/shows/${nextUpItem.show.id}`">
                Open event
              </UButton>
              <UButton variant="ghost" color="neutral" :to="`/theaters/${slug}`">
                Theater overview
              </UButton>
            </div>
          </div>
          ...
          <div v-else class="space-y-3 text-sm stage-muted">
            <p>No schedule items match the current filter combination.</p>
            <p>Shift the timeline or event filters, or add a new show if you belong to this theater.</p>
          </div>
        </UCard>
      </div>
    </StageSection>
            ...
            <div class="flex items-start justify-between gap-4">
              <div>
                ...
                <p class="mt-2 text-sm stage-muted">
                  Theater-specific date scan with markers tied to the current filters.
                </p>
              </div>

              <div class="flex items-center gap-2">
                <UButton size="xs" variant="ghost" @click="selectedDate = todayDate">
                  Today
                </UButton>
                <UButton
                  size="xs"
        ...
        <UCard>
          <template #header>
            <div>
              ...
              <p class="mt-2 text-sm stage-muted">
                What is landing on this theater date.
              </p>
            </div>
          </template>
          ...
          <div v-else class="space-y-3">
            ...
            >
              ...
              <div class="mt-4 border-2 border-[rgba(43,41,38,0.12)] bg-[rgba(251,247,239,0.72)] px-3 py-2">
    ...
    >
      <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <section class="stage-panel p-5 sm:p-6">
          <p class="stage-overline">Why this exists</p>
          <h2 class="mt-2 font-display text-3xl uppercase tracking-[0.08em]">
            Keep theater context intact
          </h2>
          <p class="mt-3 text-sm leading-7 stage-muted">
            The theater overview now gets to stay focused on what is next. This route carries the deeper scan work: month context, selected-day detail, and the full event agenda for one theater.
          </p>
        </section>

        <aside class="grid gap-3">
          <UButton block color="warning" :to="`/theaters/${slug}`" icon="i-heroicons-building-library">
            Theater overview
          </UButton>
          <UButton
            v-if="canReview"
