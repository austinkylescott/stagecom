...
2. Find unnecessary console.log statements in project/
...
7. Check if the .env.production has the same variables (not always the same value) as the .env. If something is missing, tell me. If .env.production does not exist, it means we are not in production yet.
